...
const withTheme = <P extends ContextProps = ContextProps>(Component: ComponentType<P>) => (props: Omit<P, keyof ContextProps>) => {
    const [theme, onChange] = useTheme()

    return (
        <Component {...(props as P)} theme={theme} onChange={onChange} />
    )
}
